// Enhanced DashboardHeader.jsx
import { Building, User, Bell, Settings, Search, Plus, ChevronDown } from "lucide-react";
import { useState } from "react";

export function DashboardHeader({ user, organization, activeTab, setActiveTab }) {
  const [showUserMenu, setShowUserMenu] = useState(false);
  const [showNotifications, setShowNotifications] = useState(false);

  // Quick action items that appear in header
  const quickActions = [
          ...
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">{organization?.description}</p>
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center space-x-2">
                {availableActions.map((action) => (
                  <button
                    ...
                    className="flex items-center space-x-2 px-3 py-2 text-sm font-medium text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors"
                    title={action.label}
                  >
